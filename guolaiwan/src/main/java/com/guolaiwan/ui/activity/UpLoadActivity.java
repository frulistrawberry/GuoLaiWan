...
public class UpLoadActivity extends AppCompatActivity {
        ...
        if (requestCode == 1) {
            if (data!=null && data.getData()!=null){
                ...
                try {
                    ...
                    if (Long.valueOf(durationStr)<181000){
                        img = saveBitmap(this,bitmap);
                        intent.putExtra("video_screenshot",img);
                        intent.putExtra("video_uri",path);
                        startActivity(intent);
                        finish();
                    }else {
                        Intent intent1 = new Intent(Intent.ACTION_GET_CONTENT);
                        intent1.setType("video/*");//设置类型，我这里是任意类型，任意后缀的可以这样写。
                        intent1.addCategory(Intent.CATEGORY_OPENABLE);
                        startActivityForResult(intent1,1);
                        CommonUtils.showMsg("请选择时长在3分钟以内的视频");
                    }


                }catch (Exception e){
                    CommonUtils.showMsg("请选择视频格式的文件");
                    finish();
                }
     ...
     * @param context 上下文对象
     * @param uri     图片的Uri
     * @return 如果Uri对应的图片存在, 那么返回该图片的绝对路径, 否则返回nullA
     */
